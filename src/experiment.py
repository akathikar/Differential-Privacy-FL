        ...
        for gr in range(num_global_rounds):
            ...
            with ThreadPoolExecutor(size) as pool:
                for endp in selected_endps:
                    ...
                    attack = next(iter(mal_endpoints.values()))
            ...
            gr_pca = KernelPCA(n_components=2, kernel="rbf")
            ...
            for (endp, pca_x, pca_y, pred_label) in zip(
                    ordered_endpoint_ids, pca_out[:, 0], pca_out[:, 1], pred):
                ...
                trial_results["num_malicious_endpoints"].append(num_malicious_endpoints)
                trial_results["num_flipped_label_pairs"].append(num_flipped_label_pairs)
